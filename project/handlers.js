function gameEvents(game) {
  ...
  document
    .getElementById("dropzone")
    .addEventListener("drop", (event) => onDrop(event, game));

  document
    .getElementById("soundBtn")
    .addEventListener("click", () => toggleSound(game));
...
function onDrop(event, game) {
  ...
  if (game.commands.length > 0 && document.getElementById("playTurn").disabled)
