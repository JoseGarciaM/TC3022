...
class Game {
  ...
  toggleDebug() {
    if (this.loaded) {
      ...
      if (player.boxColiderH)
        player.boxColiderH.visible = !player.boxColiderH.visible;
      game.levelObj.tiles.forEach((tile) => {
        if (tile.boxColiderH)
          tile.boxColiderH.visible = !tile.boxColiderH.visible;
      });
    }
  }
  ...
  toggleAmbienSound() {
    if (this.ambienAudio.paused) {
      ...
      document.getElementById("soundBtn").innerHTML = "Apagar Sonido";
      return true;
    } else {
      this.ambienAudio.pause();
      document.getElementById("soundBtn").innerHTML = "Prender Sonido";
      return false;
    }
  }
...
class Tile {
  ...
  update() {
    if (this.boxColider) {
      ...
      this.boxColider
        .copy(newBounds)
        .applyMatrix4(this.obj.children[0].matrixWorld);
    }
  }

  updloadSuccess(tileObj, pos) {
    tileObj.children[0].material =
      this.type != "e"
        ? game.levelObj.materialTile
        : game.levelObj.materialTileEnd;
  ...
  uploadProcessing(xhr) {
    console.log((xhr.loaded / xhr.total) * 100 + "% tile");
  }
  ...
  playTweenAnimation() {
    ...
    if (this.isSceneInTween) {
      ...
      this.frontTween = new TWEEN.Tween(this.obj.position)
        .to(
          {
            x: deepTempPos.x,
            y: 0,
...
class Player {
  ...
  updloadSuccess(robotObj) {
    ...
    if (robotObj.animations.length > 0) {
      this.actionAnim = robotObj.mixer.clipAction(
        robotObj.animations[0],
        robotObj
      );
  ...
  playTweenAnimation() {
    ...
    if (this.isFrontTween) {
      let target = new THREE.Vector3(1, 0, 0).applyQuaternion(
        playerGroup.quaternion
      );
      this.inAction = true;
      this.action = "front";
      this.frontTween = new TWEEN.Tween(playerGroup.position)
        .to(
          {
    ...
    if (this.isLeftTween) {
      ...
      this.inAction = true;
      this.action = "left";
      this.leftTween = new TWEEN.Tween(playerGroup.rotation)
        .to(
          {
    ...
    if (this.isRightTween) {
      this.inAction = true;
      this.action = "right";
      this.rightTween = new TWEEN.Tween(playerGroup.rotation)
        .to(
          {
            y: playerGroup.rotation.y - Math.PI / 2,
          },
...
class HUD {
  ...
  clearDrag() {
    while (this.draggables.firstChild)
      this.draggables.removeChild(this.draggables.lastChild);
  }

  clearDrop() {
    while (this.dropzone.firstChild)
      this.dropzone.removeChild(this.dropzone.lastChild);
  ...
  setHudDraggables() {
    ...
    game.levelObj.buttons = JSON.parse(
      JSON.stringify(game.levelsData[game.level].buttons)
    );
    this.clearDrop();
    this.clearDrag();

    for (const [key, value] of Object.entries(
      game.levelsData[game.level].buttons
    )) {
  ...
  startButton() {
    ...
    this.startBtn.innerHTML = "Empezar!";
  }
}
